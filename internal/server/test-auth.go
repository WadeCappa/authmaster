...
import (
	...
	"github.com/WadeCappa/authmaster/internal/store"
...
func (s *server) TestAuth(ctx context.Context, _ *authmaster.TestAuthRequest) (*authmaster.TestAuthResponse, error) {
	...
	token := t[0]
	testResult, err := store.Call(s.postgresUrl, func(conn *pgx.Conn) testResult {
		var userId int64
